...
const getData = async (jsonName) => { //promesa
    try {
        const respuesta = await fetch('../data'+jsonName+'.json');//Fetch: consulta link a ruta relativa. Away: espera rta de la cosulta hasta obtener un dato, sino da error. Lo que trae lo guardo en una var. Response esta en formato json ("color":"rojo")
        const data = await respuesta.json();// convierto a response de json a un [{},{}] y lo guardo en una var. Response sigue siendo una prom entonces le agrego el await 
